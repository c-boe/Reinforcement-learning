"""
Episodic Semi-gradient Sarsa (chapter 10.1) and Sarsa(lambda) (chapter 12.7) 
with binary features and linear function approximation for MontainCar 
enviroment of chapter 10.1 in Sutton and Barto's "Reinforcement Learning"'
"""
import numpy as np
...
def init_e_traces(dim_weights):
    """
    Initilize eligability trace vector

    Parameters
    ----------
    dim_weights : int
        dimension of weight vectors for linear function approximation 
...
def episodic_sg_SARSA(env, fvecs, idcs_per_action, weights, alpha, epsilon, 
                      gamma, num_actions, num_episodes):
...
def Sarsa_lbda_w_bf_and_lin_FA(env, fvecs, idcs_per_action, weights, alpha,
    ...
    (chapter 12.7) for mountain car problem"""
...
if __name__ == "__main__":
    ...
    #%%
    #episodic_sg_SARSA(env, fvecs, idcs_per_action, weights, alpha, epsilon,
    #                   gamma, num_actions, num_episodes)
    
    Sarsa_lbda_w_bf_and_lin_FA(env, fvecs, idcs_per_action, weights, alpha, 
                               epsilon, gamma, num_actions, num_episodes, lbda)
